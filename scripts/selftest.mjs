...
import { verdictOf, VERDICTS, combinedPct, ENG_GRADE_TO_PCT } from '../js/verdict.js';
...
// 3c) 영어 백분위 환산이 종합 판정(적정 여부)에 반영되는지
ok('영어등급→백분위 환산 가중합', Math.abs(combinedPct(60, 1) - (60*0.75 + ENG_GRADE_TO_PCT[1]*0.25)) < 1e-9, `combined(60,1등급)=${combinedPct(60,1)}`);
const synMeta = { admissionRound: '정시', referenceOnly: false };
const synRec = { pct50: 65, pct70: 65, eng50: 3, eng70: 3 };
const vNoEng = verdictOf(synRec, synMeta, 60, null).key;   // 국수탐만: diff 5 → 상향
const vEng1  = verdictOf(synRec, synMeta, 60, 1).key;      // 영어 1등급 반영 → 적정으로 상승
ok('영어 미반영 시 상향', vNoEng === '상향', vNoEng);
ok('영어 1등급 반영 시 적정으로 상승', vEng1 === '적정', `${vNoEng}→${vEng1}`);
const vEng9  = verdictOf(synRec, synMeta, 60, 9).key;      // 영어 9등급이면 더 불리 → 안정/상향 유지(적정 아님)
ok('영어 9등급 반영 시 적정 아님', vEng9 !== '적정', `9등급→${vEng9}`);
